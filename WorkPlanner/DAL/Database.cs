...
namespace WorkPlanner.DAL
{
    public class Database
    {
        ...
        public static List<WorkPlan> GetWorkplans(string userIds, DateTime workDate)
        {
            MySqlConnection conn = new MySqlConnection(_connectionString);
            ...
            try
            {
                ...
                string Query = "SELECT users.name, wp.task_name,wp.project_name,wp.estimated_hours,wp.user_id,wp.pinestem_task_id,wp.created_on FROM work_plans AS wp LEFT JOIN users AS users ON users.user_id = wp.user_id where wp.user_id in (" + userIds + ")  and  wp.work_date =  '" + _date + "'";
                ...
                while (reader.Read())
                {
                    ...
                    objResult.UserId = Convert.ToUInt32(reader["user_id"]);
                    objResults.Add(objResult);                 
                }
